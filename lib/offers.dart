        ...
        children: [
          ...
          if (_isScanning)
            Padding(
              ...
              child: SizedBox(
                height: 200,
                child: ClipRRect(borderRadius: BorderRadius.circular(12), child: MobileScanner(onDetect: (capture) {
              ...
              builder: (context, productSnapshot) {
                ...
                return StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, offerSnapshot) {
                    ...
                    // Group all fetched offers by product ID
                    ...
                    // 1. Primero, filtramos los productos que tienen ofertas.
                    final productsWithOffers = products.where((productDoc) {
                      return offersByProductId.containsKey(productDoc.id);
                    }).toList();

                    // 2. Luego, aplicamos el término de búsqueda sobre esa lista filtrada.
                    final filteredProducts = productsWithOffers.where((productDoc) {
                      ...
                      return _searchTerm.isEmpty || productName.contains(lowerSearchTerm) || barcode.contains(lowerSearchTerm);
                    }).toList();
...
class _OfferDialogState extends State<OfferDialog> {
  ...
  late final TextEditingController _productSearchController; // Inicializar en initState
  ...
  // Método para manejar el código de barras detectado para la búsqueda de producto en el diálogo de ofertas
  Future<void> _onBarcodeDetectedForProductSearch(String barcodeValue) async {
    if (!mounted) return;
    HapticFeedback.lightImpact();
  
    final productDoc = await widget.firestoreService.getProductByBarcode(widget.businessId, barcodeValue);
  
    if (productDoc != null) {
      final data = productDoc.data() as Map<String, dynamic>;
      setState(() {
        _selectedProductId = productDoc.id;
        _selectedProductName = data['name'] as String?;
        _productSearchController.text = _selectedProductName ?? '';
        _isScanningProductInOfferDialog = false; // Ocultar el escáner después de una detección exitosa
      });
    } else {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Producto no encontrado.')));
      setState(() {
        _isScanningProductInOfferDialog = false; // Ocultar el escáner incluso si no se encontró
      });
    }
  }

  // Widget para el escáner de código de barras dentro del diálogo de ofertas para búsqueda de producto
  Widget _buildProductSearchBarcodeScanner() {
    return SizedBox(
      height: 200,
      child: MobileScanner(
        onDetect: (capture) {
          final barcode = capture.barcodes.first.rawValue;
          if (barcode != null) _onBarcodeDetectedForProductSearch(barcode);
        },
      ),
    );
  }
    ...
    return AlertDialog(
      ...
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          child: Form( // No es necesario StatefulBuilder aquí, OfferDialog ya es un StatefulWidget
            key: _formKey,
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      icon: Icon(
                        Icons.qr_code_scanner,
                        color: _isScanningProductInOfferDialog // Usamos el nuevo estado para el escáner de ofertas
                            ? Theme.of(context).colorScheme.primary
                            : null,
                      ),
                      onPressed: () {
                        setState(() => _isScanningProductInOfferDialog = !_isScanningProductInOfferDialog);
                      },
                      tooltip: 'Escanear código de barras',
                    ),
                  ],
                ),
                // Mostrar el escáner si está activo
                if (_isScanningProductInOfferDialog) ...[
                  const SizedBox(height: 8),
                  _buildProductSearchBarcodeScanner(),
                ],
